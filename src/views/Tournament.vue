            ...
            <v-expansion-panels accordion multiple :value="[0, 1]">
              <v-expansion-panel class="mb-6">
                ...
                <v-expansion-panel-content>
                  <v-row no-gutters>
                    <v-col cols="5" offset="1" class="text-left header"
                      >Player</v-col
                    >
                    <v-col cols="3" class="text-center header"
                      >Matches Won</v-col
                    >
                  ...
                  <v-row
                    ...
                    :class="[
                      { 'player-row': i < standingsSorted.length - 1 },
                      { 'winner-row': isTopPlayer(i) },
                      { 'top-row': isTopPlayer(i) },
                    ]"
                  >
                    <v-col
                      cols="1"
                      class="data-rank justify-center"
                      align="center"
                    >
                      <h3 :class="{ 'data-top mt-1': isTopPlayer(i) }">
                        #{{ i + 1 }}
                      </h3>
                    </v-col>
                    ...
                    <v-col
                      cols="5"
                      :class="[
                        'data-name justify-center',
                        { 'data-top mt-1': isTopPlayer(i) },
                      ]"
                    >
...
export default {
  ...
  computed: {
    ...mapState(["isLoading", "stat", "tournament", "tournamentNav"]),
    standingsSorted() {
      const sortedPlayers = [];
  ...
  methods: {
    ...mapActions(["getMatches", "getStat", "getTournament"]),
    ...mapMutations([
      "setLoading",
      "setTournament",
      "setTournamentNav",
    ...
    attachGamesWon() {
      this.stats[1].doubles.forEach((statPlayer) => {
        ...
        const player = this.players.find(
          (player) => player.id === statPlayer.id
        );
        if (player) {
          player.gamesWon = {
            won: statPlayer.hits,
    ...
    displayGamesWon(num, denom) {
      ...
      return `${num}/${denom} (${getPrecisionPercentage(num, denom).toFixed(
        2
      )}%)`;
    },
    getFullName(player) {
